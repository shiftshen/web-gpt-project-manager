...
if (args.action === 'read') {
  ...
  const roundVisible = await evaluate(`JSON.stringify(document.body.innerText.includes(${JSON.stringify(args.roundId)}))`)
  ...
  if (normalize(draft.draft) !== normalize(prompt)) throw new Error('Composer text mismatch; inspect before retry')
  ...
  await page.waitForFunction(roundId => document.body.innerText.includes(roundId), args.roundId, { timeout: 12000 })
