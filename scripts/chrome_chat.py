...
return JSON.stringify({url:location.href,title:document.title,busy:b.some(e=>/^(停止回答|停止生成|Stop generating|Stop response)$/i.test(e.getAttribute('aria-label')||'')||e.dataset.testid==='stop-button'),draft:editor?(editor.innerText||editor.value||''):'',text:document.body.innerText.slice(-16000),buttons:b.slice(-30).map(e=>({text:e.innerText,aria:e.getAttribute('aria-label'),testid:e.dataset.testid})),hasEditor:!!editor});})()"""
def listing():
