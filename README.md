...
网页经理负责阶段计划、证据核验与验收；可按需读取文件或运行聚焦验证，业务代码由开发者实施和测试。阶段计划和验收条件可以按证据调整；总体目标、架构基线和预算权限仍按原授权升级。

v0.4.3 明确：经理在 stage/replan 决定中提出受控计划修订，由本地 `project.py accept` 同步，不直接覆盖控制文件。

v0.4.4 明确：默认使用 Work / GPT-6 Sol / 中；经理负责规划与验证，业务代码由本地开发者实施。模型仍须按实际界面核实。

v0.4.5 明确：本地开发者可自行修复普通错误；涉及方案、验收、权限或无法自行解决的问题，再向经理简短咨询，阶段完成后提交正式审核。
